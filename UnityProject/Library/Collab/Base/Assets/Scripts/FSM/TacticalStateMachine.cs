...
{
    /// <summary>
    /// An implementation of a tactical state machine which applies a map of 
    /// behaviors to the various states.
    /// </summary>
    public class TacticalStateMachine : StateMachine
    {
        private Dictionary<State, IStateBehavior> StateBehaviors;

        public TacticalStateMachine(State initialState, Dictionary<State, IEnumerable<Transition>> transitions) : base(initialState, transitions)
        {
        ...
        public Command GetCommand(Character agent, GameManager gameManager)
        {
            ...
            if(!StateBehaviors.TryGetValue(CurrentState, out behavior)) {
                ...
                return new Command(Vector2.zero, Vector2.zero, false, false);
            }
